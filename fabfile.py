...
def update_node_image(conn):
    ...
    conn.run('docker pull public.ecr.aws/x2t8a1o3/bankless-node:latest')
...
def send_binary(conn):
    ...
    zip_file = 'bankless-node.zip'
    cmd = f'zip -j {zip_file} bin/bankless-node'
    call(cmd.split())
    conn.put(f'{zip_file}', '.')
...
def create_dispatch_cmd(conn, pid):
    ...
    cmd = f'/home/ubuntu/bankless-node {flags} 2> {pid}.log'
...
def purge(conn, pid):
    auth = pid_to_auth(pid)
    conn.run(
        f'/home/ubuntu/bankless-node purge-chain --base-path data/{auth} --chain chainspec.json -y')
...
def stop_world(conn):
    ...
    conn.run('killall -9 bankless-node')
...
def send_new_binary(conn):
    # 1. send new binary
    send_zip(conn, 'bankless-node-new.zip', 'bin/bankless-node-new')

    # 2. make backups
    conn.run(
        'cp bankless-node bankless-node-old.backup && cp bankless-node-new bankless-node-new.backup')
...
def upgrade_binary(conn):
    ...
    conn.run('cp bankless-node bankless-node-old && cp bankless-node-new bankless-node')
