...
def launch_new_instances_in_region(n_parties=1, region_name=default_region(),
    ...
    return create_instances(region_name, image_id, n_parties, instance_type, 'bankless',
                            security_group_id, volume_size, tag)
...
def run_cmd_in_region(shcmd='ls', region_name=default_region(), tag='dev'):
    ...
    results = []
    for ip in ip_list:
        cmd = f'ssh -o "StrictHostKeyChecking no" -q -i key_pairs/bankless.pem ubuntu@{ip} -t "{shcmd}"'
