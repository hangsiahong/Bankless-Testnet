...
Bankless World Blockchain python scripts for running TestNet and DevNet.
...
- Put keys for ssh in key_pairs (both the private key e.g. `bankless.pem` and its fingerprint (`bankless.fingerprint`))
- Put SSL certificates in the nginx/cert directory (`self-signed.crt` and `self-signed.key`)
- Make sure you have the bankless node binary inside a bin directory (needed for committee key generation step, `cp <...>/bankless-node/target/release/bankless-node bin/`)
