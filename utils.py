...
def create_security_group(region_name, ip_list=[], tag=''):
    ...
    security_group_name = 'bankless-' + tag
...
def check_key_uploaded_all_regions(key_name='bankless'):
...
def generate_key_pair_all_regions(key_name='bankless'):
...
def init_key_pair(region_name, key_name='bankless', dry_run=False):
...
def generate_account():
    ...
    cmd = './bin/bankless-node key generate --output-type json --words 24'
...
def bootstrap_chain(account_ids, chain, benchmark_config=None, **custom_flags):
    ...
    cmd = './bin/bankless-node bootstrap-chain --base-path data'
    if chain == 'dev':
        cmd += f' --chain-id a0dnet1 --n-members {len(account_ids)}'
    else:
        flags = {
            '--chain-id': 'a0tnet1',
            '--chain-name': 'BanklessWorldTestnet',
    ...
    # TODO tmp workaround
    if chain != 'dev':
        chainspec['name'] = 'Bankless World Testnet'
    ...
    if benchmark_config is not None:
        prepare_benchmark_accounts(chainspec, benchmark_config['n_of_accounts'],
                                  benchmark_config['bankless_amount'])
...
def prepare_vesting(chainspec):
    ...
    balances = []
    vesting = []
    hundred_bankless = 100 * bankless()
    day = 86400
    for i, aid in enumerate(vested_accounts[:12]):
        mod = i+1
        balances.append((aid, hundred_bankless))
        vesting.append((aid, mod, mod*day, mod*bankless()))

    month = 2592000
    for i, aid in enumerate(vested_accounts[12:]):
        mod = i+1
        balances.append((aid, 100*hundred_bankless))
        vesting.append((aid, mod, mod*month, mod*hundred_bankless))
...
def prepare_benchmark_accounts(chainspec, n_of_accounts, bankless_amount):
    n_of_accounts = int(n_of_accounts)
    bankless_amount = int(bankless_amount)

    bench_accounts = generate_accounts_from_paths((str(i) for i in range(n_of_accounts)))
    
    amount = bankless_amount * bankless()
...
def generate_p2p_keys(account_ids):
    ...
    for auth in account_ids:
        cmd = f'./bin/bankless-node key generate-node-key --file data/{auth}/p2p_secret'
...
def fab_cmd():
    ...
    return f'fab -i key_pairs/bankless.pem {fabfile_path_flag}'
